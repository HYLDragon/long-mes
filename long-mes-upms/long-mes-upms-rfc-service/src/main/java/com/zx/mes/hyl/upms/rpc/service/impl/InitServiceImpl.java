...
import com.zx.mes.hyl.upms.dao.*;
import com.zx.mes.hyl.upms.model.*;
...
public class InitServiceImpl implements InitServiceI {
	...
	private void initRole() {
		...
		Trole barAdmin = new Trole();
		barAdmin.setId("barAdmin");
		barAdmin.setName("条码相关管理员");
		barAdmin.setTrole(superAdmin);// 父级是超管
		barAdmin.setSeq(6);
		barAdmin.getTresources().addAll(resourceDao.find("from Tresource t where t.tresource.id in ('bargl') or t.id in ('bargl')"));
	...
	private void initUser() {
		List<Tuser> l = userDao.find("from Tuser t where t.name in ('华云龙 ','admin1','admin2','admin3','admin4','admin5','guest')");
		if (l != null && l.size() > 0) {
			for (Tuser user : l) {
				userDao.delete(user);
			}
